...
class Solution{
  public:
    ...
    // If one of the trees is empty, but the other is not, they are not isomorphic.
    if (root1 == nullptr || root2 == nullptr) {
        return false;
    }

    // If the root values are different, the trees are not isomorphic.
    if (root1->data != root2->data) {
        return false;
    }

    // Check isomorphic for the following two cases:
    // 1. Flipping both subtrees.
    // 2. Not flipping any subtree.
